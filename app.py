...
# 按标签搜索单词
@app.route('/tags/<int:tag_id>/words', methods=['GET'])
def get_words_by_tag(tag_id):
    conn = get_db_connection()
    if not conn:
        return jsonify({"error": "Database connection failed"}), 500
...
def delete_tag(tag_id):
    ...
    try:
        with conn.cursor() as cur:
            # First delete tag associations
            cur.execute('DELETE FROM word_tags WHERE tag_id = %s', (tag_id,))
            
            # Then delete the tag
            cur.execute('DELETE FROM tags WHERE id = %s RETURNING id', (tag_id,))
